"""Input and output helpers for the NUTS calibration skill."""
...
def write_samples_csv(path: Path, records: list[dict[str, Any]], field_order: list[str] | None = None) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    fieldnames = list(field_order or (records[0].keys() if records else []))
...
def _load_csv_payload(path: Path) -> tuple[np.ndarray, list[str]]:
    ...
    with path.open("r", encoding="utf-8", newline="") as handle:
        ...
        if has_header:
            ...
            data = [[float(row[h]) for h in headers] for row in rows]
            return np.asarray(data, dtype=float), headers
