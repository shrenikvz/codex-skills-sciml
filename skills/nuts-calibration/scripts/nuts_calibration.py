...
from nuts_calibration_lib.analysis import (
    SUPPORTED_BACKENDS,
...
from nuts_calibration_lib.config import clone_default_config, load_config
from nuts_calibration_lib.environment import probe_environment
from nuts_calibration_lib.inference import run_calibration
from nuts_calibration_lib.project import create_project, inspect_model_inputs
...
def cmd_list_capabilities(_args: argparse.Namespace) -> dict[str, object]:
    return {
        ...
        "likelihoods": SUPPORTED_LIKELIHOODS,
        "priors": SUPPORTED_PRIORS,
...
def cmd_run(args: argparse.Namespace) -> dict[str, object]:
    ...
    if args.backend:
        cfg["algorithm"]["backend"] = args.backend
    if args.device:
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="General-purpose NUTS calibration workflow")
    sub = p.add_subparsers(dest="subcommand", required=True)

    sub.add_parser("doctor", help="Check backend imports and visible devices")
    sub.add_parser("list-capabilities", help="List backends, priors, likelihoods, transforms, and plots")
    sub.add_parser("show-template", help="Print the default config template")

    inspect_model = sub.add_parser("inspect-model", help="Inspect model, data, and NUTS recommendations")
    ...
    inspect_model.add_argument("--likelihood", default="auto")
    inspect_model.add_argument("--backend", default="auto")
    inspect_model.add_argument("--plot", action="append", default=[])

    create = sub.add_parser("create-project", help="Create a runnable NUTS calibration project")
    ...
    create.add_argument("--likelihood", default="auto")
    create.add_argument("--backend", default="auto")
    create.add_argument("--plot", action="append", default=[])
    create.add_argument("--warmup-steps", type=int, default=None)
    ...
    create.add_argument("--step-size", type=float, default=None)
    create.add_argument("--overwrite", action="store_true")

    run = sub.add_parser("run", help="Run NUTS calibration from a config.json")
